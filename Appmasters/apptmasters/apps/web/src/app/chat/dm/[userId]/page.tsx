...
export default function DMPage() {
  ...
  const [otherUser, setOtherUser] = useState<{ name: string; color?: string } | null>(null);
  ...
  async function load() {
    ...
    const res = await fetch(`${API_URL}/api/comms/dm/${userId}`, {
  ...
  useEffect(() => { load(); loadOtherUser(); }, [token, userId]);
  ...
  async function send() {
    if (!text.trim()) return;
    setSending(true);
    await fetch(`${API_URL}/api/comms/dm/${userId}`, {
      method: "POST",
      ...
      <header className="bg-white border-b border-gray-100 px-4 py-4 sticky top-0 z-10">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="font-bold text-gray-900">{otherUser?.name ?? "Direct Message"}</h1>
